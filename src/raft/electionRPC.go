...
func (rf *Raft) AttemptElection() {
	...
	for !rf.killed() {
		...
		for i, _ := range peers {
			...
			args.LastLogIndex = 0
			if rf.maxLogIndex() > 0 {
				args.LastLogIndex = rf.maxLogIndex()
				args.LastLogTerm = rf.getLatestTerm()
			}
...
func (rf *Raft) CheckLatestLog(args *RequestVoteArgs) bool {
	...
	Debug(dVote, "Candidate[%d]log checked by[%d] ,args:%+v, rf.latest term:%d,rf.latestindex:%d",
		args.CandidateId, rf.me, args, rf.Logs[rf.log2sliceIndex(rf.maxLogIndex())].Term, rf.maxLogIndex())
	if args.LastLogTerm > rf.getLatestTerm() {
		return true
	} else if args.LastLogTerm < rf.getLatestTerm() {
		return false
	} else {
		return args.LastLogIndex >= rf.maxLogIndex()
	}
